...
Class_Potential_RxSM_OS_physparams_nG_ndT::Class_Potential_RxSM_OS_physparams_nG_ndT(
    const ISMConstants &smConstants)
    : Class_Potential_Origin(smConstants)
{
  Model         = ModelID::ModelIDs::RXSM_OS_PHYSPARAMS_NG_NDT;

  nPar = 5;   // number of parameters in the tree-Level Lagrangian AFTER using
               // tadpole equations
  nParCT = 5; // number of parameters in the counterterm potential
 ...
 */
std::vector<std::string> Class_Potential_RxSM_OS_physparams_nG_ndT::addLegendCT() const
{
 ...
 * input file
 */
std::vector<std::string> Class_Potential_RxSM_OS_physparams_nG_ndT::addLegendTemp() const
{
  std::vector<std::string> labels;
...
Class_Potential_RxSM_OS_physparams_nG_ndT::addLegendTripleCouplings() const
{
  std::vector<std::string> labels;
 ...
 */
std::vector<std::string> Class_Potential_RxSM_OS_physparams_nG_ndT::addLegendVEV() const
{
  std::vector<std::string> labels;
  // out = "Your VEV order"
  labels.push_back("wh");
 ...
 */
void Class_Potential_RxSM_OS_physparams_nG_ndT::ReadAndSet(const std::string &linestr,
 ...
 */
void Class_Potential_RxSM_OS_physparams_nG_ndT::set_gen(const std::vector<double> &par)
{
  ...
  mh = (pow(m1,2)*vh*pow(cos(a),2) - (pow(m1,2) - pow(m2,2))*vs*cos(a)*sin(a) + pow(m2,2)*vh*pow(sin(a),2))/(2.*vh); 
  ms = (pow(m2,2)*vs*pow(cos(a),2) - (pow(m1,2) - pow(m2,2))*vh*cos(a)*sin(a) + pow(m1,2)*vs*pow(sin(a),2))/(2.*vs); 
 ...
 */
void Class_Potential_RxSM_OS_physparams_nG_ndT::set_CT_Pot_Par(const std::vector<double> &par)
{
  dmh = par[0];
  dms = par[1];
  dLh = par[2];
  ...
  Curvature_Higgs_CT_L2[1][1] = -dms;

  Curvature_Higgs_CT_L4[0][0][0][0] = dLh;
  Curvature_Higgs_CT_L4[0][0][1][1] = 2*dLhs;
  Curvature_Higgs_CT_L4[0][1][0][1] = 2*dLhs;
  Curvature_Higgs_CT_L4[0][1][1][0] = 2*dLhs;
  Curvature_Higgs_CT_L4[1][0][0][1] = 2*dLhs;
  Curvature_Higgs_CT_L4[1][0][1][0] = 2*dLhs;
  Curvature_Higgs_CT_L4[1][1][0][0] = 2*dLhs;
  Curvature_Higgs_CT_L4[1][1][1][1] = dLs;
 ...
 */
void Class_Potential_RxSM_OS_physparams_nG_ndT::write() const
{
  std::stringstream ss;
 ...
 */
std::vector<double> Class_Potential_RxSM_OS_physparams_nG_ndT::calc_CT() const
{
  ...
  parCT.push_back(-0.5*(vh*HesseWeinberg(0,0) + vs*HesseWeinberg(0,1) - 3*NablaWeinberg(0))/vh); //dmh;
  parCT.push_back(-0.5*(vh*HesseWeinberg(0,1) + vs*HesseWeinberg(1,1) - 3*NablaWeinberg(1))/vs); //dms;
  parCT.push_back((-3*vh*HesseWeinberg(0,0) + 3*NablaWeinberg(0))/pow(vh,3)); //dLh;
  parCT.push_back((-3*vs*HesseWeinberg(1,1) + 3*NablaWeinberg(1))/pow(vs,3)); //dLs;
  parCT.push_back(-0.5*HesseWeinberg(0,1)/(vh*vs)); //dLhs;

  return parCT;
}

// mass basis triple couplings
void Class_Potential_RxSM_OS_physparams_nG_ndT::TripleHiggsCouplings()
{
  if (!SetCurvatureDone) SetCurvatureArrays();
...
void Class_Potential_RxSM_OS_physparams_nG_ndT::SetCurvatureArrays()
{
  initVectors();
  ...
  Curvature_Higgs_L4[0][0][1][1] = 2*Lhs;
  Curvature_Higgs_L4[0][1][0][1] = 2*Lhs;
  Curvature_Higgs_L4[0][1][1][0] = 2*Lhs;
  Curvature_Higgs_L4[1][0][0][1] = 2*Lhs;
  Curvature_Higgs_L4[1][0][1][0] = 2*Lhs;
  ...
  Curvature_Lepton_F2H1[0][1][0] = SMConstants.C_MassElectron/vh;
  Curvature_Lepton_F2H1[1][0][0] = SMConstants.C_MassElectron/vh;
  Curvature_Lepton_F2H1[2][3][0] = SMConstants.C_MassMu/vh;
  Curvature_Lepton_F2H1[3][2][0] = SMConstants.C_MassMu/vh;
  Curvature_Lepton_F2H1[4][5][0] = SMConstants.C_MassTau/vh;
  Curvature_Lepton_F2H1[5][4][0] = SMConstants.C_MassTau/vh;
...
bool Class_Potential_RxSM_OS_physparams_nG_ndT::CalculateDebyeSimplified()
{
  return false;
  /*
...
bool Class_Potential_RxSM_OS_physparams_nG_ndT::CalculateDebyeGaugeSimplified()
{
  /*
...
double
Class_Potential_RxSM_OS_physparams_nG_ndT::VTreeSimplified(const std::vector<double> &v) const
{
  if (not UseVTreeSimplified) return 0;
  double res = 0;
  res = (pow(v[0],4)*Lh + pow(v[1],4)*Ls + 12*pow(v[0],2)*(pow(v[1],2)*Lhs - mh) - 12*pow(v[1],2)*ms)/24.;
  
  return res;
}

double Class_Potential_RxSM_OS_physparams_nG_ndT::VCounterSimplified(
    const std::vector<double> &v) const
{
  if (not UseVTreeSimplified) return 0;
  double res = 0;
  res = (pow(v[0],4)*dLh + pow(v[1],4)*dLs + 12*pow(v[0],2)*(pow(v[1],2)*dLhs - dmh) - 12*pow(v[1],2)*dms)/24.;
  
  return res;
}

void Class_Potential_RxSM_OS_physparams_nG_ndT::Debugging(const std::vector<double> &input,
