...
(* ::Chapter:: *)
(*In this section the scalar potential is defined. The model is the \[AliasDelimiter] (my conventions).*)
...
par = {mh,ms,Lh,Ls,Lhs,dLhOS,dLsOS,dLhsOS};
...
VHiggs = -par[[1]]/2*C11 - par[[2]]/2*C22  + (par[[3]]+par[[6]])/24*C11^2 + \
		 + (par[[4]]+par[[7]])/24*C22^2 + (par[[5]]+par[[8]])/2*C11*C22//Simplify
...
InputParameters={Lh,Ls,Lhs,vh,vs,dLhOS,dLsOS,dLhsOS};
...
(DepententParameters = Flatten[{TadpoleRep//Simplify}])//TableForm
...
(*Define the counterterm potential, =0 in MSb + dT in this case*)
VCT = parCT[[(par//Length)+1;;]] . higgsbase
...
CTs={0,0,0,0,0,0,0,0,-NCW[1],-NCW[2]}
...
VF = (DL . Dd . DR)Phih[[1]] - (UL . DU . UR)Conjugate[Phih[[1]]]//Simplify
...
ImplementModel[
"RxSM_MS_lagparams_nG_dT_cts", (*Model name*)
...
(*Lh,Ls,Lhs,vh,vs,dLhOS,dLsOS,dLhsOS *)
{0.918, 0.618, 0.045, 246.22, 650,0,0,0},(*Specific input parameters for the example file*)
